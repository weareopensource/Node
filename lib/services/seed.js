...
const seedTheUser = (UserService, user) => async (password) => {
  ...
  if (
    user.email === seedOptions.seedAdmin.email &&
    process.env.NODE_ENV === 'production' &&
    (await UserService.get(user))
  )
    return new AppError(
      `Failed due to local account already exists: ${user.email}`,
    );
  ...
  try {
    ...
    if (seedOptions.logResults)
      console.log(
        chalk.bold.red(
          `Database Seeding:\t\t\tLocal ${user.email} added with password set to ${password}`,
        ),
      );
    return result;
  } catch (err) {
...
const seedTasks = async (TaskService, task, user) => {
  try {
    ...
    if (seedOptions.logResults)
      console.log(
        chalk.bold.red(`Database Seeding:\t\t\tLocal ${task.title} added`),
      );
    return result;
  } catch (err) {
  ...
  try {
    if (process.env.NODE_ENV === 'production') {
      ...
      if (process.env.NODE_ENV === 'development') {
        result.push(
          await seedTasks(TaskService, seedOptions.seedTasks[0], result[0]),
        );
        result.push(
          await seedTasks(TaskService, seedOptions.seedTasks[1], result[1]),
        );
      }
    }
  } catch (err) {
