  ...
  if (_.includes(config.joi.supportedMethods, method)) {
    ...
    if (result && result.error) {
      if (
        ...
        (result.error.original.password || result.error.original.firstname)
      )
        result.error.original = _.pick(
          result.error.original,
          config.whitelists.users.default,
        );
      let description = '';
      result.error.details.forEach((err) => {
        description += `${
          err.message.charAt(0).toUpperCase() +
          err.message.slice(1).toLowerCase()
        }. `;
      });

      if (
        result.error._original &&
        (result.error._original.password || result.error._original.firstname)
      )
