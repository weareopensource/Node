...
const logConfiguration = () => {
    ...
    if (config.cors.origin.length > 0)
        console.log(chalk.green(`Cors:            ${config.cors.origin}`));
};
// Boot up the server
exports.start = async () => {
    let db;
    ...
    try {
        if (config.secure && config.secure.credentials)
            ...
            http = await nodeHttp.createServer(app).setTimeout(config.api.timeout).listen(config.api.port, config.api.host);
        logConfiguration();
        return {
            db,
            orm,
