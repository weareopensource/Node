...
exports.updateAvatar = async (req, res) => {
    try {
        // catch multerErr
        if (req.multerErr)
            throw new AppError(req.multerErr.message, { code: 'SERVICE_ERROR', details: req.multerErr });
        // delete old image
        if (req.user.avatar)
        ...
        req.login(user, (errLogin) => {
            if (errLogin)
                return responses.error(res, 400, 'Bad Request', errors.getMessage(errLogin))(errLogin);
