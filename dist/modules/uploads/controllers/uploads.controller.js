...
exports.get = async (req, res) => {
    try {
        ...
        if (!stream)
            responses.error(res, 404, 'Not Found', 'No Upload with that identifier can been found')();
        stream.on('error', (err) => {
...
exports.uploadByName = async (req, res, next, uploadName) => {
    try {
        ...
        if (!upload)
            responses.error(res, 404, 'Not Found', 'No Upload with that name has been found')();
        else {
            req.upload = upload;
...
exports.uploadByImageName = async (req, res, next, uploadImageName) => {
    try {
        ...
        if (!upload)
            return responses.error(res, 404, 'Not Found', 'No Upload with that name has been found')();
        // options
        const sharp = _.get(config, `uploads.${upload.metadata.kind}.sharp`);
        if (opts[1] && (!sharp || !sharp.sizes))
            return responses.error(res, 422, 'Unprocessable Entity', 'Size param not available')();
        ...
        if (opts[2] && (!sharp || !sharp.operations))
            return responses.error(res, 422, 'Unprocessable Entity', 'Operations param not available')();
        if (opts[2] && !sharp.operations.includes(opts[2]))
            return responses.error(res, 422, 'Unprocessable Entity', 'Operation param not available')();
        // return
        req.upload = upload;
        if (upload.metadata && upload.metadata.user)
            req.isOwner = upload.metadata.user; // user id if we proteck road by isOwner policy
