  ...
  describe('Logged', () => {
    ...
    test('should not be able to register a new user with weak password', async () => {
      ...
      try {
        ...
        expect(result.body.description).toEqual(
          'Password must have a strength of at least 3. "password" length must be at least 8 characters long. ',
        );
      } catch (err) {
        console.log(err);
        expect(err).toBeFalsy();
    ...
    test('if should not be able to register, should not return sensible data', async () => {
      ...
      try {
        ...
        expect(
          JSON.parse(result.body.error)._original.password,
        ).toBeUndefined();
      } catch (err) {
        console.log(err);
        expect(err).toBeFalsy();
    ...
    test('should be able to delete a single user if admin', async () => {
      ...
      try {
        const result = await agent
          .delete(`/api/users/${userEdited._id}`)
          .expect(200);
        expect(result.body.type).toBe('success');
    ...
    test('forgot password should return 400 for non-local provider set for the user object', async () => {
      ...
      try {
        ...
        expect(result.body.message).toBe('Bad Request');
        expect(result.body.description).toBe(
          `It seems like you signed up using your ${userEdited.provider} account`,
        );
      } catch (err) {
        console.log(err);
        expect(err).toBeFalsy();
      ...
      try {
        ...
        try {
          const result2 = await agent
            .get(`/api/auth/reset/${result.resetPasswordToken}`)
            .expect(302);
          expect(result2.headers.location).toBe(
            `/password/reset/${result.resetPasswordToken}`,
          );
        } catch (err) {
          console.log(err);
          expect(err).toBeFalsy();
    ...
    test('should not be able to change profile avatar if attach a avatar with a different field name', async () => {
      try {
        const result = await agent
          ...
          .attach(
            'fieldThatDoesntWork',
            './modules/users/tests/img/default.png',
          )
          .expect(422);
        expect(result.body.message).toEqual('Unprocessable Entity');
    ...
    test('should not be able to upload a non-image file as a profile avatar', async () => {
      try {
        ...
        expect(result.body.message).toEqual('Unprocessable Entity');
        expect(result.body.description).toEqual(
          'Only image/png,image/jpeg,image/jpg,image/gif images allowed.',
        );
      } catch (err) {
        console.log(err);
        expect(err).toBeFalsy();
