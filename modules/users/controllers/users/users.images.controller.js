...
const UploadsService = require(path.resolve(
  './modules/uploads/services/uploads.service',
));
...
exports.updateAvatar = async (req, res) => {
  try {
    // catch multerErr
    if (req.multerErr)
      throw new AppError(req.multerErr.message, {
        code: 'SERVICE_ERROR',
        details: req.multerErr,
      });
    // delete old image
    if (req.user.avatar)
      await UploadsService.delete({ filename: req.user.avatar });
    // update document uploaded (metadata ...)
    const result = await UploadsService.update(req.file, req.user, 'avatar');
    // update user
    const user = await UserService.update(req.user, {
